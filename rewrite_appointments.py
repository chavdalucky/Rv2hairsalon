    ...
    const unsub = onSnapshot(q, async (snap) => {
      ...
      for (const b of filtered) {
         if (['pending', 'confirmed'].includes((b.status || '').toLowerCase())) {
            if (b.date) {
               // Simple check: if date is in the past
      ...
      if (!updatedSome) {
          ...
          // Find first active appointment for the top card
...
content = re.sub(r"  useEffect\(\(\) => \{.*?\}, \[userId, userPhone, userEmail\]\);", new_effect, content, flags=re.DOTALL)
...
# Remove the duplicated reschedule/cancel buttons in the normal list for active appointments?
# The requirements say "remove the active appointment card from the main screen view", maybe we don't render it in the regular list if it's the active one.
# So we filter out the active appointment from the mapped list.
...
# Handle Expired state rendering and Re-book prompt
