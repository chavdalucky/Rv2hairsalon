...
import { Calendar, Clock, Scissors, ArrowRight, User, Edit2, X } from 'lucide-react';
...
export default function UpcomingAppointment() {
      ...
      if (user) {
        ...
        const unsub = onSnapshot(q, (snap) => {
          if (!snap.empty) {
            ...
            setAppointments(docs.slice(0, 5));
          } else {
            setAppointments([]);
          }
          setLoading(false);
  ...
  if (loading || appointments.length === 0 || dismissed) return null;
    ...
    >
      <div className="w-full bg-gradient-to-r from-zinc-900 to-black border border-amber-500/30 rounded-2xl p-6 md:p-8 shadow-[0_0_30px_rgba(245,158,11,0.1)] relative overflow-hidden group min-h-[220px] md:min-h-[140px] flex items-center">
        <button 
          ...
          className="absolute top-2 right-2 sm:top-4 sm:right-4 z-30 p-2 text-zinc-500 hover:text-white hover:bg-zinc-800 rounded-full transition-colors"
          aria-label="Dismiss"
        >
          <X className="w-4 h-4" />
        </button>
